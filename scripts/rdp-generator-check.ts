 ...
 * JSON round-trip, детерминизм. Остается в репо.
 *
...
console.log(`[3] generateField на старте НЕ решен (${RUNS} прогонов)`);
for (let i = 0; i < RUNS; i += 1) {
  assert(!checkSolution(generateField(6, 1)), `generateField(6,1) #${i} не решен на старте`);
  assert(!checkSolution(generateField(7, 2)), `generateField(7,2) #${i} не решен на старте`);
}

console.log('[4] Guard симметрии STRAIGHT: поле из «одних прямых» не остается решенным');
{
  ...
  assert(checkSolution(straightField), 'straightField (все 0°) решен до перемешивания');
  const shuffled = shuffleRotations(straightField, Math.random);
  assert(shuffled !== null, 'shuffleRotations нашел нерешенную расстановку прямых');
  if (shuffled) {
    assert(!checkSolution(shuffled), 'перемешанные прямые не остаются решенными (симметрия учтена)');
  }
}
...
for (let i = 0; i < RUNS; i += 1) {
  ...
  assert(compA.has(tileId(solved.exits[0].row, solved.exits[0].col)), `сц.2 #${i}: путь A соединен`);
  assert(compB.has(tileId(solved.exits[1].row, solved.exits[1].col)), `сц.2 #${i}: путь B соединен`);
...
console.log('[6] Негатив: rotate-all-90 на решенном поле → false');
...
console.log('[8] Негатив (edge): коннектор «в стену» / без встречного — не соединяет');
{
  ...
  assert(!checkSolution(broken), 'edge: коннектор в стену не дает соединения → false');
}
...
// --- Сценарий 1: decoy-обманки, изолированные от пути ------------------------

console.log(
  `[12] Сц.1 — есть decoy-обманки, не замкнутые в решение (${RUNS} прогонов)`,
);
for (let i = 0; i < RUNS; i += 1) {
  const solved = buildSolvedField(6, 1);

  // Решение цело несмотря на обманки.
  assert(checkSolution(solved), `сц.1 #${i}: путь решаем при обманках`);

  // reachable из входа = клетки пути; обманки = непустые вне reachable.
  const reachable = reachableCells(solved, solved.entries[0]);
  const nonEmpty = solved.tiles.filter((t) => t.type !== 'EMPTY');
  const decoys = nonEmpty.filter((t) => !reachable.has(t.id));

  assert(decoys.length > 0, `сц.1 #${i}: обманки присутствуют`);
  // decoys по определению не входят в reachable → «мусор не замкнут в путь».
}
