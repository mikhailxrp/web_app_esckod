...
export interface PuzzleField {
  ...
  exits: GridPosition[];
  /**
   * Суммарная длина «настоящих» путей (без decoy-обманок), заданная при генерации.
   * Не выдает, КАКИЕ клетки — путь (сами decoy'и остаются неразличимы для игрока),
   * только их итоговое количество — знаменатель для прогресс-бара (см. computePuzzleProgress).
   * Опционально: старые сохраненные поля и demo-фикстуры его не содержат.
   */
