...
- Открытая плашка — раскрывает inline-панель взломщика вместо секции плашек миссий
- Поле отображает зашифрованный текст из 80 кликабельных слов: 25-30 сессионных (из `CrackSession.wordList`) + заполнители из `wordList5letters`. Клик по слову → попадает в поле ввода «Ключ». Клик по заполнителю не тратит попытку, но показывает тематический ответ «Ложная цель. Шум в системе.»
...
**Игровой опыт:** игрок видит шифротекст с 80 словами и вводит слово в поле «Ключ» (либо кликает по слову в шифротексте — оно автоматически попадает в поле). После каждой попытки — цветовая подсветка букв относительно `targetWord`. Это даёт обучение через попытки и держит игрока в напряжении.
...
- 25-30 слов в сессии (`CrackSession.wordList`) — сервер генерирует при старте
- UI отображает 80 слов в шифротексте: сессионные + заполнители из `wordList5letters`
