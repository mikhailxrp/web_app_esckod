...
- Чат Марины **скрыт до серверного триггера** (миссия RDP сценарий 2 → автоактивация в `/file-viewed` после изучения всех доступных файлов)
...
**Кто пишет:** сервер в `POST /api/missions/rdp/[slotKey]/file-viewed` для слотов с `rdpScenario === 2`, в момент автоматической активации триггера (когда игрок просмотрел все доступные файлы). См. `missions-rdp.md` → раздел «Шаг 4 — сюжетный триггер».
...
Особый TRIGGER: после прохождения сюжетной точки (миссия RDP сценарий 2 → автоактивация триггера в `/file-viewed` по факту просмотра всех доступных файлов) сервер устанавливает `GameProgress.marinaTriggered = true`. Клиент при следующем `GET /api/progress` видит флаг и **рендерит чат Марины** в dashboard.
...
**Стилизация под голосовое:** на старте — нативный `<audio controls>`. Если заказчик попросит точную копию интерфейса Telegram (волновая диаграмма) — добавим `wavesurfer.js`. Это решается на этапе подключения дизайн-системы (Фаза 11).
...
`marinaTriggered` устанавливается в `true` сервером в `POST /api/missions/rdp/[slotKey]/file-viewed` при автоматической активации триггера — но только для слотов с `rdpScenario === 2`. Триггер срабатывает, когда игрок просмотрел все файлы во всех когда-либо доступных папках. См. `missions-rdp.md` → раздел «Шаг 4 — сюжетный триггер».
